...
public class GerenciadorDuelos {
    ...
    // ... (Os métodos mais simples como agendarDuelo, buscarDueloPorId, etc. não foram comentados em detalhe por serem autoexplicativos) ...
    ...
    private void atualizarCondicaoDeCampo(Duelo duelo) {
        // nextDouble() gera um número entre 0.0 e 1.0. Se for menor que 0.3, a condição muda (30% de chance).
        if (random.nextDouble() < 0.3) {
            // Cria uma lista com todas as condições de campo possíveis.
            String[] condicoes = {CondicaoCampo.TEMPESTADE_ARCANA, CondicaoCampo.CHUVA_ELEMENTAL, CondicaoCampo.NUVEM_SOMBRIA, CondicaoCampo.NORMAL};
            // Sorteia uma nova condição da lista.
            String novaCondicao = condicoes[random.nextInt(condicoes.length)];
            
            // Verifica se a nova condição é diferente da atual para evitar mensagens repetidas.
    ...
    private void simularDueloEquipes(Duelo duelo) {
        ...
        // O loop principal do duelo. Continua enquanto houver magos vivos em ambas as equipes.
        while (!equipe1Vivos.isEmpty() && !equipe2Vivos.isEmpty() && rodada <= 10) {
            ...
            // Processa efeitos de status (como queima de mana, veneno, etc.) em todos os magos antes dos turnos.
            for (Mago mago : todosOsVivos) { mago.processarEfeitosDoTurno(); }

            // Embaralha a lista de magos para que a ordem dos turnos seja aleatória a cada rodada.
        ...
        // Quando o loop termina (uma equipe foi derrotada ou o tempo acabou), define o vencedor.
        determinarVencedor(duelo);
    }
    
    private void simularDuelo1v1(Duelo duelo) {
        // A lógica do 1v1 é similar, mas mais simples, sem a necessidade de listas complexas.
    ...
    private void executarAcaoDoMago(Mago atacante, List<Mago> inimigosVivos, Duelo duelo) {
        ...
        if (atacante.getControlador().equals(Mago.IA)) {
            // Pede para a IA do Mago escolher um feitiço baseado em sua estratégia (ex: Agressivo).
        ...
        // ---- LÓGICA DO JOGADOR HUMANO ----
            ...
            // Pega o feitiço escolhido da lista (ajustando o índice, pois a lista começa em 0).
     ...
     * Considera poder base, bônus de arena, bônus de condição de campo e resistência do alvo.
    ...
    private void aplicarDano(Mago atacante, List<Mago> alvos, Feitico feitico, Duelo duelo) {
        ...
        for (Mago alvo : alvos) {
            ...
            duelo.setUltimoAtacante(alvo, atacante); // Registra quem foi o último a atacar este alvo (para assistências).
    ...
    private Mago escolherAlvoAleatorio(List<Mago> inimigos) { return inimigos.get(random.nextInt(inimigos.size())); }

    /**
     * Determina o vencedor do duelo após o fim do combate.
     * A condição primária é ter sobreviventes. A secundária é o total de dano causado.
    ...
    /**
     * Gera e exibe um ranking de magos baseado em abates e, como critério de desempate, dano total.
     */
    public void gerarRankingDeMagos() {
        System.out.println("\n--- RANKING GERAL DE MAGOS ---");
        // Um 'Map' para armazenar as estatísticas de cada mago. A chave é o Mago, o valor é um array [abates, dano].
        ...
        // Ordena a lista. O critério principal é o número de abates (stats[0]), em ordem decrescente.
...
/**
     ...
     * Este mapa é um histograma que mostra a quantidade de duelos agendados para cada hora do dia,
    ...
    public void gerarMapaDeCalorDeHorarios() {
        ...
        // Verificação inicial: se não houver duelos, informa o usuário e encerra a função.
        ...
        // Cada posição representará uma hora do dia e armazenará a contagem de duelos naquela hora.
        ...
        for(int i = 0; i < 24; i++) {
            // Imprime a etiqueta da hora formatada. O "%02d" garante que a hora tenha dois dígitos (ex: "07:00" em vez de "7:00").
            System.out.printf("%02d:00 | ", i);
            
            // Este é o loop que desenha a "barra de calor".
            // Ele vai repetir o número de vezes correspondente à contagem de duelos para a hora 'i'.
            for(int j = 0; j < horas[i]; j++) { 
                // A cada repetição, imprime um caractere de bloco, construindo a barra visual.
                System.out.print("■"); 
            }
            
            // Após desenhar a barra, imprime a contagem numérica e quebra a linha para a próxima hora.
