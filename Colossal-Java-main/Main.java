...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (executando) {
            ...
            switch (opcao) {
                case 1: menuGerirMagos(scanner, gerenciadorMagos); break;
                case 2: menuGerirDuelos(scanner, gerenciadorMagos, gerenciadorDuelos); break;
                case 3: menuRelatorios(scanner, gerenciadorDuelos); break;
    ...
    private static void menuGerirMagos(Scanner scanner, GerenciadorMagos gm) {
        ...
        int opcao = lerInteiro(scanner);
        switch(opcao) {
            case 1: cadastrarMago(scanner, gm); break;
            case 2: gm.listarMagos(); break;
            case 0: return;
            default: System.out.println("Opção inválida.");
        }
    }
    ...
    private static void menuGerirDuelos(Scanner scanner, GerenciadorMagos gm, GerenciadorDuelos gd) {
        ...
        int opcao = lerInteiro(scanner);
        switch (opcao) {
            case 1: agendarDuelo(scanner, gm, gd); break;
            case 2: listarDuelos(gd, "AGENDADOS"); break;
            case 3: simularDueloAgendado(scanner, gd); break;
            case 4: simularTreino(scanner, gm, gd); break;
            case 0: return;
            default: System.out.println("Opção inválida.");
        }
    }
    ...
    private static void menuRelatorios(Scanner scanner, GerenciadorDuelos gd) {
        ...
        int opcao = lerInteiro(scanner);
        switch(opcao) {
            case 1: gd.gerarRankingDeMagos(); break;
            case 2: gd.gerarTaxaDeVitoriaPorEscola(); break;
            case 3: gd.gerarMapaDeCalorDeHorarios(); break;
            case 4: listarDuelos(gd, "FINALIZADOS"); break;
            case 0: return;
            default: System.out.println("Opção inválida.");
        }
    }
    ...
    private static void cadastrarMago(Scanner scanner, GerenciadorMagos gerenciador) {
        System.out.println("\n--- Cadastro de Mago ---");
        System.out.print("ID (número único): "); int id = lerInteiro(scanner);
        if (gerenciador.buscarMagoPorId(id) != null) { System.out.println("Erro: ID já existe!"); return; }
        System.out.print("Codinome: "); String codinome = scanner.nextLine();
        System.out.println("Escola: 1-Arcano, 2-Elemental, 3-Sombrio"); int esc = lerInteiro(scanner);
        System.out.println("Foco: 1-Vara, 2-Cajado, 3-Tomo"); int f = lerInteiro(scanner);
        String foco = f == 1 ? Mago.VARA : (f == 2 ? Mago.CAJADO : Mago.TOMO);
        System.out.println("Controlador: 1-Humano, 2-IA"); int c = lerInteiro(scanner);
        String ctrl = c == 1 ? Mago.HUMANO : Mago.IA;
        String perfilIA = ctrl.equals(Mago.IA) ? Mago.IA_OPORTUNISTA : null;
        if(ctrl.equals(Mago.IA)){
            System.out.println("Perfil da IA: 1-Agressivo, 2-Oportunista (Padrão)"); int p = lerInteiro(scanner);
            perfilIA = p == 1 ? Mago.IA_AGRESSIVO : Mago.IA_OPORTUNISTA;
        }
        System.out.print("Mana Máxima: "); int mana = lerInteiro(scanner);
        System.out.print("Poder Base: "); double poder = lerDouble(scanner);
        System.out.print("Resistência Mágica: "); double res = lerDouble(scanner);
        Mago novoMago = null;
        switch (esc) {
            case 1: novoMago = new MagoArcano(id, codinome, mana, poder, res, foco, ctrl, perfilIA); break;
            case 2: novoMago = new MagoElemental(id, codinome, mana, poder, res, foco, ctrl, perfilIA); break;
            case 3: novoMago = new MagoSombrio(id, codinome, mana, poder, res, foco, ctrl, perfilIA); break;
            default: System.out.println("Escola inválida!"); return;
        }
        if(gerenciador.cadastrarMago(novoMago)) System.out.println("\n==> Mago '" + codinome + "' cadastrado com sucesso!");
    }
    ...
    private static void agendarDuelo(Scanner scanner, GerenciadorMagos gerMagos, GerenciadorDuelos gerDuelos) {
        ...
        if (gerMagos.getQuantidadeMagos() < tamanhoEquipe * 2) { System.out.println("Não há magos suficientes para este modo!"); return; }
        gerMagos.listarMagos();
        List<Mago> equipe1 = montarEquipe(scanner, gerMagos, "Equipe 1", tamanhoEquipe, new ArrayList<>());
        if(equipe1 == null) { System.out.println("Agendamento cancelado."); return; }
        List<Mago> equipe2 = montarEquipe(scanner, gerMagos, "Equipe 2", tamanhoEquipe, equipe1);
        if(equipe2 == null) { System.out.println("Agendamento cancelado."); return; }
        System.out.println("Arena: 1-Círculo Rúnico, 2-Labirinto Ilusório, 3-Câmara Antimagia");
        int arenaNum = lerInteiro(scanner);
        String arena = arenaNum == 1 ? Duelo.CIRCULO_RUNICO : (arenaNum == 2 ? Duelo.LABIRINTO_ILUSORIO : Duelo.CAMARA_ANTIMAGIA);
        LocalDateTime dataHora = null;
        while (dataHora == null) {
            System.out.print("Data e Hora do Duelo (formato dd/MM/yyyy HH:mm) ou 'agora': ");
            String dataStr = scanner.nextLine();
            if(dataStr.equalsIgnoreCase("agora")) { dataHora = LocalDateTime.now(); break; }
            try { dataHora = LocalDateTime.parse(dataStr, FORMATADOR_DATA); } catch (DateTimeParseException e) { System.out.println("Formato de data inválido! Tente novamente."); }
        }
    ...
    private static void simularTreino(Scanner scanner, GerenciadorMagos gm, GerenciadorDuelos gd) {
        ...
        while(jogadorEscolhido == null) {
            ...
            if(jogadorEscolhido == null) System.out.println("ID inválido!");
        }

        // ======================= INÍCIO DA CORREÇÃO =======================
        //
        // Criamos uma nova variável 'final' que recebe o valor final de 'jogadorEscolhido'.
        ...
        Duelo treino = new Duelo("Treino", List.of(jogadorFinal), List.of(oponenteIA), Duelo.CIRCULO_RUNICO, LocalDateTime.now());
    ...
    private static void listarDuelos(GerenciadorDuelos gd, String tipo) {
        ...
        System.out.println("\n--- Duelos " + tipo + " ---");
        if (lista.isEmpty()) { System.out.println("Nenhum duelo para mostrar."); }
        else { lista.forEach(System.out::println); }
    }

    private static List<Mago> montarEquipe(Scanner scanner, GerenciadorMagos gerMagos, String nomeEquipe, int tamanho, List<Mago> jaEscolhidos){
    ...
    private static int lerInteiro(Scanner scanner) {
        while (!scanner.hasNextInt()) { System.out.print("Entrada inválida. Por favor, digite um número inteiro: "); scanner.next(); }
        int numero = scanner.nextInt(); scanner.nextLine(); return numero;
    }

    private static double lerDouble(Scanner scanner) {
        while (!scanner.hasNextDouble()) { System.out.print("Entrada inválida. Por favor, digite um número (ex: 10,5): "); scanner.next(); }
        double numero = scanner.nextDouble(); scanner.nextLine(); return numero;
    }
...
- Utiliza submenus para organizar as diferentes funcionalidades (Gestão de Magos, Duelos, Relatórios).
- Interage com as classes 'GerenciadorMagos' e 'GerenciadorDuelos' para executar as ações
