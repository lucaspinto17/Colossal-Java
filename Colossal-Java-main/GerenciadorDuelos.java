...
public class GerenciadorDuelos {
    ...
    private void executarAcaoDoMago(Mago atacante, List<Mago> inimigosVivos, Duelo duelo) {
        ...
        Feitico feitico = atacante.escolherFeitico(inimigosVivos);
        if (feitico == null) {
        ...
        atacante.consumirMana(feitico.getCustoMana());
        System.out.printf("%s lança '%s'!%n", atacante.getCodinome(), feitico.getNome());
        if (feitico.getTipo().equals(Feitico.PROJETIL)) {
            aplicarDano(atacante, List.of(escolherAlvoAleatorio(inimigosVivos)), feitico, duelo);
        } else if (feitico.getTipo().equals(Feitico.AREA)) {
            System.out.printf("  > O feitiço atinge toda a Equipe Inimiga!%n");
            aplicarDano(atacante, inimigosVivos, feitico, duelo);
        }
    }
...
- É o cérebro da lógica de negócios do sistema de duelos.
- Gere a lista de duelos agendados, permitindo adicionar novos duelos e evitando conflitos.
- Contém a lógica de simulação para os modos 1v1, 3v3 e Treino.
- O ciclo de simulação processa os turnos, ações dos magos, aplicação de dano, efeitos e abates.
- Inclui os métodos para gerar todos os relatórios: Ranking, Taxa de Vitória e Mapa de Calor.
=============================================================================================
*/